...
function runDataTables() {
    ...
    var selectable = $('#datatable_col_reorder input').length > 0;

    if (selectable) {
        /*
         $('#datatable_col_reorder tbody tr').click( function() {
         var row= $(this);
         row.toggleClass('row-selected');
         var newStatus=!row.find("input")[0].checked;
         row.find("input")[0].checked=newStatus;
         if(!newStatus)
         $('#datatable_col_reorder thead input')[0].checked=false;
         }
         */


        var allElement = $('#datatable_col_reorder thead input');
        var canSelectAll = allElement.length > 0;
        var all = false;

        $('#datatable_col_reorder tbody input').click(function () {
            var item = $(this);
            var newStatus = item[0].checked;
        ...
        if (canSelectAll) {
            allElement.click(function () {
                all = allElement[0].checked;
                $('#datatable_col_reorder tbody input').each(
                    function (index, ele) {
                        ele.checked = all;
                    }
                );
