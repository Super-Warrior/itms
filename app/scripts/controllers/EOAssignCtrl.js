angular.module("itmsApp").controller("eoAssignCtrl", ["$scope", "$modal", "$log", "$http", "config", "common", "customerService",
   function ($scope, $modal, $log, $http, config, common, customerService) {
      $scope.module = "计划";
      $scope.title = "需求分配";
      $scope.columns = [
      ...
      $scope.adjust = function () {
         ...
         customerService.searchCustomer("car").then(function (result) {
            $scope.carriers = result.data;
            $scope.modalInstance = $modal.open({
               templateUrl: "views/planning/adjustDeliveryMethod.html",
               scope: $scope
            });
         });
      };
